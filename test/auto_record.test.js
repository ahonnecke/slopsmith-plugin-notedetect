// Auto-record tests — every play is captured for later analysis without
// the user arming anything (the teaching-tool goal). The behaviour:
...
test('isAutoRecord defaults true; setAutoRecord toggles it', () => {
...
test('a failed auto-save preserves the take — neither song:loaded nor song:play re-arms over it', async () => {
    const core = loadDetectionCore();
    const det = core.createNoteDetector({ isDefault: true });
    det._bindAutoRecord(); // auto-record is on by default on this branch
